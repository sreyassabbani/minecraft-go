...
void Gyro::update() {
    ...
    Vector<3> a_body_mps2 { (ax_raw / accelLsbPerG) * G0,
                            (ay_raw / accelLsbPerG) * G0,
                            (az_raw / accelLsbPerG) * G0 };
    Vector<3> g_body_rads {
        (gx_raw / gyroLsbPerDPS) * (M_PI / 180.0f) - gyroBias[0],
        (gy_raw / gyroLsbPerDPS) * (M_PI / 180.0f) - gyroBias[1],
        (gz_raw / gyroLsbPerDPS) * (M_PI / 180.0f) - gyroBias[2]
    };
    ...
    linearAccelWorld =
        Vector<3> { a_world_mps2[0], a_world_mps2[1], a_world_mps2[2] - G0 };
    ...
    float gyroMag_dps = sqrtf(powf(gx_raw / gyroLsbPerDPS, 2) +
    ...
    if (isStill) {
        ...
        velocity = Vector<3> { velocity[0] + linearAccelWorld[0] * dt,
                               velocity[1] + linearAccelWorld[1] * dt,
                               velocity[2] + linearAccelWorld[2] * dt };
        // mild damping to limit drift growth
        velocity =
            Vector<3> { velocity[0] * velDamping, velocity[1] * velDamping,
                        velocity[2] * velDamping };
    }

    position = Vector<3> { position[0] + velocity[0] * dt,
                           position[1] + velocity[1] * dt,
                           position[2] + velocity[2] * dt };
}

Vector<3> Gyro::getLinearAcceleration() const { return linearAccelWorld; }

Vector<3> Gyro::getOrientationEuler() const { return quatToEuler(q); }

Vector<3> Gyro::getPosition() const { return position; }

Vector<3> Gyro::getVelocity() const { return velocity; }
