...
ST7796S tft = ST7796S(TFT_CS, TFT_DC, TFT_RST);

// SoftSPI - note that on some processors this might be *faster* than hardware
// SPI!
// ST7796S tft = ST7796S(TFT_CS, TFT_DC, MOSI, SCK, TFT_RST, MISO);
...
unsigned long testText() {
    ...
    tft.println(
        F("So she was considering in her own mind (as well as she could, for "
    ...
    tft.println(
        F("There was nothing so very remarkable in that; nor did Alice think "
    ...
    tft.println(F("In another moment down went Alice after it, never once "
                  "considering how in the world she was to get out again."));

    tft.println(F("The rabbit-hole went straight on like a tunnel for some "
    ...
    tft.println(
        F("Either the well was very deep, or she fell very slowly, for she had "
...
unsigned long testCircles(uint8_t radius, uint16_t color) {
    ...
    start = micros();
    for (x = 0; x < w; x += r2) {
        for (y = 0; y < h; y += r2) { tft.drawCircle(x, y, radius, color); }
    }
...
unsigned long testRoundRects() {
    ...
    for (i = 0; i < w; i += 8) {
        i2 = i / 2 - 2;
        tft.drawRoundRect(cx - i2, cy - i2, i, i, i / 8,
                          tft.color565(i, 100, 100));
    }
...
unsigned long testFilledRoundRects() {
    unsigned long start;
    int i, i2, cx = tft.width() / 2 + 10, cy = tft.height() / 2 + 10;

    tft.fillScreen(ST7796S_BLACK);
    start = micros();
    for (i = min(tft.width(), tft.height()) - 20; i > 25; i -= 6) {
        i2 = i / 2;
        tft.fillRoundRect(cx - i2, cy - i2, i - 20, i - 20, i / 8,
                          tft.color565(100, i / 2, 100));
    }
