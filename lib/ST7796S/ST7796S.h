/*
 * ST7796S class inherits from Adafruit_GFX, Adafruit_SPITFT class and the Arduino Print class.
 * ST7796S written by David Prentice
 *
 * Any use of ST7796S class and examples is dependent on Adafruit and Arduino licenses
...
class ST7796S : public Adafruit_SPITFT {
public:
  ST7796S(int8_t _CS, int8_t _DC, int8_t _MOSI, int8_t _SCLK,
                   int8_t _RST = -1, int8_t _MISO = -1);
  ST7796S(int8_t _CS, int8_t _DC, int8_t _RST = -1);
#if !defined(ESP8266)
  ST7796S(SPIClass *spiClass, int8_t dc, int8_t cs = -1,
                   int8_t rst = -1);
#endif // end !ESP8266
  ST7796S(tftBusWidth busWidth, int8_t d0, int8_t wr, int8_t dc,
