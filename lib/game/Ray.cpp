...
RaycastHit raycast(const algebra::Vector<3>& start, const algebra::Vector<3>& dir, float maxDist, const World& world) {
    ...
    float sideDistX = (stepX > 0) ? (mapX + 1.0f - x) * deltaDistX : (x - mapX) * deltaDistX;
    float sideDistY = (stepY > 0) ? (mapY + 1.0f - y) * deltaDistY : (y - mapY) * deltaDistY;
    float sideDistZ = (stepZ > 0) ? (mapZ + 1.0f - z) * deltaDistZ : (z - mapZ) * deltaDistZ;
    ...
    while (dist < maxDist) {
        ...
        if (world.inBounds(mapX, mapY, mapZ)) {
            if (world.isSolid(world.getBlock(mapX, mapY, mapZ))) {
                ...
                // Determine face
                if (side == 0) result.face = (stepX > 0) ? 0 : 1; // hit from left (-x) or right (+x) side? Wait.
                // If stepX > 0, we are moving in +x direction. We hit the left face of the block (which faces -x).
                // So if stepX > 0, face is -x (0). If stepX < 0, face is +x (1).
                if (side == 0) result.face = (stepX > 0) ? 0 : 1;
                else if (side == 1) result.face = (stepY > 0) ? 2 : 3; // 2: -y, 3: +y
