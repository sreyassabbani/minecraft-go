...
Camera::Camera(const Vec3& pos, const Vec3& lookDirection, float fovDegrees,
    ...
    float len = sqrtf(lookDir[0] * lookDir[0] + lookDir[1] * lookDir[1] +
                      lookDir[2] * lookDir[2]);
    if (len > 0.0001f) {
        lookDir[0] /= len;
        lookDir[1] /= len;
...
Mat4 Camera::pointAt(const Vec3& pos, const Vec3& target,
                     const Vec3& up) const {
    // Calculate new forward direction
    Vec3 newForward = target - pos;
    float len =
        sqrtf(newForward[0] * newForward[0] + newForward[1] * newForward[1] +
              newForward[2] * newForward[2]);
    if (len > 0.0001f) {
        newForward[0] /= len;
        newForward[1] /= len;
        newForward[2] /= len;
    ...
    // a = newForward dot up
    float a =
        newForward[0] * up[0] + newForward[1] * up[1] + newForward[2] * up[2];
    Vec3 newUp;
    ...
    len =
        sqrtf(newUp[0] * newUp[0] + newUp[1] * newUp[1] + newUp[2] * newUp[2]);
    if (len > 0.0001f) {
        newUp[0] /= len;
        newUp[1] /= len;
        newUp[2] /= len;
